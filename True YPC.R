#full_pbp = load_pbp(1999:2023)
pbp = load_pbp(2021:most_recent_season())
stats = load_player_stats(1999:most_recent_season())

pbp %>%
  filter(season == 2023, rush_attempt == 1) %>% 
  mutate(true_yards_gained = if_else(yards_gained > 10, 10, yards_gained))  %>%
  group_by(defteam) %>%
  summarize(
    true_ypc = mean(true_yards_gained, na.rm = T),
    .groups = "drop"
  ) %>%
  arrange(true_ypc) %>%
  View()
...
pbp %>%
  filter(season == 2023, pass_attempt == 1, week > 11) %>% 
  mutate(
    true_yards_gained = if_else(yards_gained > 16, 16, yards_gained),
    adj_yards_gained = if_else(touchdown == 1, 20, 0) + yards_gained + if_else(interception == 1, -45, 0)
    )  %>%
  group_by(defteam) %>%
  summarize(
    true_ypa = mean(yards_gained, na.rm = T),
    aya = sum(adj_yards_gained)/n(),
    .groups = "drop"
  ) %>%
  arrange(true_ypa) %>%
  View()
...
stats %>% 
  filter(season == 2023) %>%
  group_by(week, recent_team) %>%
  mutate(team_fp = sum(fantasy_points_ppr)) %>%
  select(recent_team, leading_receiver = player_display_name, week, rec_yds = receiving_yards, fpts = fantasy_points_ppr, team_fp) %>%
  arrange(week, -fpts) %>%
  group_by(recent_team, week) %>%
  top_n(2, fpts) %>%
  group_by(leading_receiver, recent_team) %>% 
  count() %>% arrange(-n) %>% view
